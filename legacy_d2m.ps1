...
$legacyFilterPath = Join-Path $scriptRoot "filters\legacy_heading_map.lua"
$compactFilterPath = Join-Path $scriptRoot "filters\docx_to_compact.lua"
...
if (-not (Test-Path -LiteralPath $legacyFilterPath)) {
  throw "Missing filter: $legacyFilterPath"
}

if (-not (Test-Path -LiteralPath $compactFilterPath)) {
  throw "Missing filter: $compactFilterPath"
}
...
& pandoc `
  -f "docx+styles" `
  -t "markdown" `
  --lua-filter="$legacyFilterPath" `
  --lua-filter="$compactFilterPath" `
  "$resolvedInput" `
  -o "$resolvedOutput"
